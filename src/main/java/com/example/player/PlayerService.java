...
public class PlayerService implements PlayerRepository {
    ...
    @Override 
    public Player createPlayerDetails(Player player){
    ...
    public Player getPlayerDetails(int playerId){
        Player playerDetails = team.get(playerId);
    ...
    public Player updatePlayerDetails(int playerId, Player player){
        ...
        if(player.getJerseyNumber() != 0){
            playerDetails.setJerseyNumber(player.getJerseyNumber());
        }

        if(player.getRole() != null){
            playerDetails.setRole(player.getRole());
        }
        return playerDetails;
    }
